...
void draw_window(App *app, Window *window) {
  ...
  for (int i = window->scroll; i < window->files.last_index; i++) {
    ...
    // mvwprintw(window->curses_win, filename_draw_y, 1, "%s",
    // filename_trimmed);
...
int main(int argc, char **argv) {
  ...
  while (user_input != 'q') {
    draw(&app);
    getmaxyx(app.winmgr.active_window->curses_win, active_window_sizeY,
             active_window_sizeX);
    ...
    if (user_input == KEY_DOWN && app.winmgr.active_window->files.filenames &&
        app.winmgr.active_window->files.last_index - 1 >
            app.winmgr.active_window->highlight) {
      ...
      int last_visible_file_idx =
          window_limit + app.winmgr.active_window->scroll;
    ...
    if (user_input == KEY_UP && app.winmgr.active_window->files.filenames &&
        app.winmgr.active_window->highlight >= 1) {
      app.winmgr.active_window->highlight -= 1;
      // Scroll
